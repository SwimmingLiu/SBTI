...
- 页面结构：介绍页 → 题目页 → 结果页
- 数据来源：题库、类型库、维度解释、图片映射、结果计算逻辑均内嵌在页面脚本中
...
从 0 到 1 完整复刻原站，保留全部题目、文案、结果、图片与判定逻辑；UI 允许轻度优化；完成本地浏览器级验证、GitHub PR 流程和 Vercel 部署。
...
- `scripts/capture-remote-results.mjs`：通过 Playwright 打开原站，逐个渲染/截图所有结果状态
